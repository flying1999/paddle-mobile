...
inline void format_bias(Tensor* bias, Tensor* scale_bias) {
    ...
    for (int i = 0; i < repeat; i++) {
      for (int j = 0; j < length; j++) {
        ...
        scale_bias_data[i * length + j] = value;
      }
    }
}
...
inline int fill_split_arg(const ConvParam& c_param) {
  ...
  for (int n = 0; n < filter->shape().num(); n++) {
    int hwc = filter->shape().channel() * filter->shape().height() * filter->shape().width();
    float* filter_data_start = filter_data + n * hwc;
    for (int i = 0; i < hwc; ++i) {
      filter_data_start[i] = filter_data_start[i] * scale_data[n];
    }
  }
